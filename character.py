...
#客服服务
class CustomerServiceChain(LLMChain):
    """Chain for customer service experts to determine the next stage of a customer service conversation."""

    @classmethod
    def from_llm(cls, llm: BaseLLM, verbose: bool = True) -> LLMChain:
        customer_service_inception_prompt_template = (

            """You are a customer service expert, assisting your team in identifying the next step in a customer service conversation based on the history provided.
    
            Following '===' is the customer's query or issue.
            ...
            Now, determine the next step by selecting only from the following options:
    
            1. Inquiry Response: Provide information or assistance requested by the customer.
    
            2. Problem Resolution: Offer a solution or steps to resolve the customer’s issue.
    
            3. Escalation: Escalate the issue to a higher level of support for complex problems.
    
            4. Feedback Collection: Ask for customer feedback or satisfaction rating regarding the service provided.
    
            5. Follow-up: Schedule a follow-up contact to ensure the customer's issue is fully resolved.
    
            6. Closure: Politely conclude the conversation after ensuring the customer's satisfaction.
    
            Choose the most appropriate step based on the conversation history.
    
            Your answer should be one number between 1 through 6, corresponding to the next step.
            """

        )

        prompt = PromptTemplate(
            template=customer_service_inception_prompt_template,
            input_variables=["conversation_history"],
        )
